...
The FoodSave AI system uses a SQLite database with SQLAlchemy ORM for data persistence. The database stores user conversations, receipts, products, recipes, and system metadata. This guide covers the database schema, models, configuration, and management.
...
class BaseRepository:
    ...
    async def get(self, db: Session, id: str) -> Optional[Base]:
        """Get record by ID"""
    ...
    async def update(self, db: Session, id: str, obj_in: Dict[str, Any]) -> Optional[Base]:
        """Update record"""
        await db.execute(
            update(self.model)
...
class ConversationRepository(BaseRepository):
    ...
    async def get_user_conversations(self, db: Session, user_id: str,
    ...
    async def add_message(self, db: Session, conversation_id: str,
...
class ReceiptRepository(BaseRepository):
    ...
    async def get_user_receipts(self, db: Session, user_id: str,
...
async def get_conversation_with_messages(db: Session, conversation_id: str) -> Conversation:
...
async def get_recent_conversations(db: Session, user_id: str,
...
# Aggregation query
async def get_user_spending_stats(db: Session, user_id: str,
...
def test_engine():
    ...
    test_db_url = "sqlite+aiosqlite:///./test_foodsave.db"
    engine = create_engine(test_db_url)

    # For SQLite, we create tables directly
