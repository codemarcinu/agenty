...
from infrastructure.vector_store.vector_store_impl import (
...
class ProcessQueryUseCase:
    ...
    async def execute(self, query: str, user_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Note: get_by_id requires session parameter, but we don't have one
            # This is a placeholder - in real implementation, you'd need to pass a session
            user = await self.user_repository.get_by_id(None, int(user_id))
            if not user:
                logger.warning(f"User not found: {user_id}")
                return {"success": False, "error": "User not found", "query": query}
            logger.info(
                f"User found: {user.get('username', 'Unknown') if isinstance(user, dict) else 'Unknown'}"
            )
            ...
            # Note: get_by_user_id method doesn't exist in the repository interface
            # Using search method as fallback
            food_items = await self.food_item_repository.search(
                None, f"user_id:{user_id}"
            )
