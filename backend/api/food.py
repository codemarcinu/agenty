...
async def add_shopping_trip(
    trip: shopping_schemas.ShoppingTripCreate, db: AsyncSession = Depends(get_db)  # type: ignore
) -> shopping_schemas.ShoppingTrip:
    """
    Dodaje nowy paragon z listą produktów do bazy danych.
    """
    db_trip = await shopping_service.create_shopping_trip(db=db, trip=trip)
    return shopping_schemas.ShoppingTrip.model_validate(db_trip)
...
async def read_shopping_trips(
    skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)  # type: ignore
) -> list[shopping_schemas.ShoppingTrip]:
    """
    Pobiera listę ostatnich paragonów z bazy danych.
    """
    db_trips = await shopping_service.get_shopping_trips(db=db, skip=skip, limit=limit)
    return [shopping_schemas.ShoppingTrip.model_validate(trip) for trip in db_trips]
...
async def update_shopping_trip(
    ...
    return shopping_schemas.ShoppingTrip.model_validate(updated_trip)
...
async def update_product(
    ...
    return shopping_schemas.ProductSchema.model_validate(updated_product)


@router.delete("/shopping-trips/{trip_id}", status_code=200)
async def delete_shopping_trip(
    trip_id: int, db: AsyncSession = Depends(get_db)  # type: ignore
) -> dict[str, str]:
    """
...
async def delete_product(
    product_id: int, db: AsyncSession = Depends(get_db)  # type: ignore
) -> dict[str, str]:
    """
