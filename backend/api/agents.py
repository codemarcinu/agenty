...
from core.database import get_db_with_error_handling
...
class OrchestratorRequest(BaseModel):
    ...
    use_perplexity: bool | None = False
    use_bielik: bool | None = True  # Domyślnie używamy Bielika
...
async def execute_orchestrator_task(
    ...
    try:
        ...
        agent_response = await orchestrator.process_command(
            ...
            session_id=session_id,
            agent_states=request.agent_states,
            use_perplexity=request.use_perplexity or False,
            use_bielik=request.use_bielik or True,
        )
...
async def initialize_mmlw() -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to initialize MMLW model")
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error initializing MMLW model: {e!s}"
        )
...
async def test_mmlw_embedding(text: str = Form(...)) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error testing MMLW embeddings: {e!s}"
        )


@router.post("/perplexity/test")
async def test_perplexity_api(query: str = Form(...)) -> dict[str, Any]:
    """Perplexity API has been removed from the system"""
    raise HTTPException(
        status_code=400,
        detail="Perplexity API has been removed from the system - using DuckDuckGo fallback",
    )
