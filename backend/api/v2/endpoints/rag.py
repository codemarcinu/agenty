...
from api.v2.exceptions import BadRequestError, UnprocessableEntityError
...
async def upload_document_to_rag(
    ...
    try:
        ...
        logger.info(f"Adding background task for file: {file.filename}")
        ...
        logger.info(f"Background task added for file: {file.filename}")
...
async def process_document_background(
    ...
    logger.info(f"Starting background processing of document: {filename}")
    ...
    try:
        ...
        logger.info(f"File exists, size: {Path(file_path).stat().st_size} bytes")

        # Przetwarzanie dokumentu
        logger.info(f"Calling rag_processor.process_file for: {file_path}")
        result = await rag_processor.process_file(file_path, metadata)
        logger.info(f"Document processing completed: {result}")

        # Sprawdź wynik
        if result.get("success"):
            logger.info(
                f"Successfully processed document with {result.get('chunks_processed', 0)} chunks"
            )
        else:
        ...
        logger.exception(f"Full traceback for document {filename}:")
        # Nie rzucaj wyjątku, tylko zaloguj błąd
        logger.error(f"Background task failed for {filename}: {e!s}")
...
async def search_documents(
    ...
    filter_type: str | None = Query(None, description="Filter by document type"),
...
async def process_directory(
    ...
    file_extensions: str | None = Query(
