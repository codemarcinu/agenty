...
# GPU-accelerated vector store
try:
    from core.faiss_gpu_service import EnhancedVectorStoreGPU

    GPU_VECTOR_STORE_AVAILABLE = True
except ImportError:
    GPU_VECTOR_STORE_AVAILABLE = False
    EnhancedVectorStoreGPU = None
...
try:
    ...
    logging.getLogger(__name__).warning(
        "LangChain not available, falling back to basic document loading"
    )
...
try:
    ...
    logging.getLogger(__name__).warning(
        "FAISS not available, falling back to simple vector store"
    )
...
try:
    ...
    logging.getLogger(__name__).warning(
        "Pinecone not available, falling back to local vector store"
    )
...
class RAGDocumentProcessor:
    ...
    def __init__(
        ...
        """
        # Use GPU vector store if available, otherwise fallback to CPU
        if vector_store:
            self.vector_store = vector_store
        elif GPU_VECTOR_STORE_AVAILABLE:
            logger.info("Using global GPU-accelerated vector store")
            from core.vector_store import vector_store as global_vector_store

            self.vector_store = global_vector_store
        else:
            logger.warning("Using global CPU vector store")
            from core.vector_store import vector_store as global_vector_store
    ...
    async def embed_text(self, text: str) -> list[float]:
        ...
        3. Ollama embeddings with Polish-optimized model
        ...
        if MMLW_AVAILABLE and settings.USE_MMLW_EMBEDDINGS:
            try:
                ...
                if mmlw_client.is_available():
                    ...
                    if embedding:
                        logger.debug("Using MMLW embeddings for Polish text")
                        return embedding
            except Exception as e:
                logger.warning(f"Failed to use MMLW embeddings: {e}")

        # Try local SentenceTransformers with Polish model
        ...
        # Try Ollama embeddings with Polish-optimized model
        try:
            import ollama

            # Use Polish-optimized model for embeddings
            polish_embedding_model = "SpeakLeash/bielik-4.5b-v3.0-instruct:Q8_0"

            response = await asyncio.to_thread(
                ollama.embeddings, model=polish_embedding_model, prompt=text
            )
            logger.debug("Using Ollama Polish embeddings")
            return response["embedding"]
        except Exception as e:
            logger.warning(f"Failed to use Ollama embeddings: {e}")

        # Fall back to hybrid_llm_client with Polish model
        try:
            embedding = await hybrid_llm_client.embed(
                text=text, model="SpeakLeash/bielik-4.5b-v3.0-instruct:Q8_0"
            )
            logger.debug("Using hybrid LLM client Polish embeddings")
            return embedding
        except Exception as e:
    ...
    def _get_loader_for_file(self, file_path: str) -> BaseLoader | None:
    ...
    def load_document(self, file_path: str | Path) -> list[str | dict[str, Any]]:
        """
    ...
    async def process_document(
        ...
        """Process document with parallel chunking and embedding"""
        try:
            # Parallel chunking
            chunks = await asyncio.create_task(
                asyncio.to_thread(self.chunk_text, content)
            )
            ...
            for i in range(0, len(chunks), batch_size):
                ...
                embedding_tasks = []
                for chunk in batch:
                    task = asyncio.create_task(self.embed_text(chunk))
                        ...
                        if self.vector_store:
                            await asyncio.create_task(
                                self.vector_store.add_document(
                                    chunk_id, embedding, chunk, metadata or {}
                                )
                            )
            ...
            logger.error(f"Error processing document {source_id}: {e}")
            return []
    ...
    async def process_directory(
        ...
        # Get all files
        files = list(path.glob("**/*")) if recursive else list(path.glob("*"))
    ...
    async def get_document_info(self, doc_hash: str) -> dict[str, Any] | None:
        ...
        if weak_ref:
            ...
            logger.debug(f"Cleaned up invalid weak reference for document: {doc_hash}")
        return None
