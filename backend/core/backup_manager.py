...
class BackupManager:
    ...
    async def create_full_backup(self, backup_name: str | None = None) -> BackupResults:
        """
    ...
    async def _backup_vector_store(self, backup_name: str) -> BackupComponentResult:
        ...
        try:
            ...
            if (
                hasattr(settings, "VECTOR_STORE_PATH")
                and Path(settings.VECTOR_STORE_PATH).exists()
            ):
    ...
    async def _find_external_drive(self) -> str | None:
    ...
    async def restore_backup(
        self, backup_name: str, components: list[str] | None = None
    ) -> dict[str, Any]:
