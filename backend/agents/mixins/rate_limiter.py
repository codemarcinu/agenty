...
class RateLimiter:
    ...
    async def check_limit(
        self, agent_type: str, user_id: str | None = None, tokens: int = 1
    ) -> bool:
        """
        ...
        ) and not await self.user_limits[agent_type][user_id].consume(tokens):
            return False

        # Check global limit
        return not (
            agent_type in self.global_limits
            and not await self.global_limits[agent_type].consume(tokens)
        )


def rate_limited(
    agent_type: str, user_id_key: str | None = None
