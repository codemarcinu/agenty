...
from core.anti_hallucination_decorator_optimized import with_weather_validation
...
class WeatherAlert(BaseModel):
    ...
    effective: datetime | None = None  # When alert starts
    expires: datetime | None = None  # When alert ends
...
class WeatherAgent(BaseAgent):
        ...
        try:
            ...
            for provider in self.providers:
                ...
                try:
                    ...
                    logger.error(f"Error with provider {provider.name}: {e!s}")
    ...
    async def _extract_location(self, query: str, model: str) -> str:
        ...
        prompt = (
            f"Wyodrębnij nazwę miasta z zapytania: '{query}'\n\n"
            f"Odpowiedz TYLKO nazwą miasta po polsku."
        )
        ...
        try:
            response = await hybrid_llm_client.chat(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Jesteś asystentem, który wyodrębnia nazwy miast. Odpowiadaj TYLKO nazwą miasta po polsku.",
                    },
        ...
        try:
            ...
            for alert_data in data.get("alerts", {}).get("alert", []):
                alert = WeatherAlert(
                    ...
                    effective=datetime.fromisoformat(alert_data.get("effective", "")),
                    expires=datetime.fromisoformat(alert_data.get("expires", "")),
    ...
    async def _get_from_cache(self, cache_key: str) -> WeatherData | None:
    ...
    def _format_weather_text(
        ...
        if has_severe_alerts:
            return "UWAGA, są ważne alerty pogodowe! Oto prognoza: " + weather_summary
        ...
        try:
            stream = await self.llm_client.chat(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Jesteś asystentem pogodowym. Podsumuj prognozę pogody po polsku. Maksymalnie 2-3 zdania.",
                    },
                    {"role": "user", "content": prompt},
                ],
                stream=True,
            ...
            yield "Przepraszam, wystąpił błąd podczas pobierania prognozy pogody."
    ...
    async def _get_coordinates(self, city: str) -> dict[str, float] | None:
    ...
    async def _fetch_mock_weather(
        ...
        """Fallback method when no weather providers are available"""
        logger.warning(f"No weather providers available for location: {location}")
        return None
