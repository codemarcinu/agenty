...
class CookingPreferences(BaseModel):
    """User cooking preferences for culinary personalization"""
    ...
    favorite_cuisines: list[str] = []  # np. "polska", "włoska", "azjatycka"
    dietary_restrictions: list[str] = (
        []
    )  # np. "wegetariański", "bezglutenowy", "bezlaktozowy"
    allergies: list[str] = []  # np. "orzechy", "skorupiaki", "jajka"
    spice_tolerance: str = "medium"  # "mild", "medium", "hot", "very_hot"

    # Stylu gotowania
    cooking_style: list[str] = []  # np. "szybko", "dokładnie", "eksperymentalnie"
    preferred_meal_types: list[str] = (
        []
    )  # np. "jednogarnkowe", "sałatki", "zupy", "dania główne"
    cooking_time_preference: str = (
        "medium"  # "quick" (<30min), "medium" (30-60min), "long" (>60min)
    )

    # Sprzęt kuchenny
    available_appliances: list[str] = (
        []
    )  # np. "piekarnik", "mikrofala", "grill", "frytkownica", "robot kuchenny"
    cooking_methods: list[str] = (
        []
    )  # np. "smażenie", "pieczenie", "gotowanie na parze", "grillowanie"

    # Inne preferencje
    budget_conscious: bool = False
    healthy_eating_focus: bool = False
    environmental_conscious: bool = False
    loves_trying_new_things: bool = True
