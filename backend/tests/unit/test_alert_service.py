...
class TestAlertService:
    ...
    async def test_send_alert(self, service) -> None:
        with (
            patch.object(service, "should_alert", return_value=True),
            patch(
                "backend.agents.adapters.alert_service.logger.warning"
            ) as mock_warning,
        ):
            await service.send_alert("test", ErrorSeverity.HIGH, {"error": "test"})
            mock_warning.assert_called_once()

    @pytest.mark.asyncio
    async def test_send_alert_not_triggered(self, service) -> None:
        with (
            patch.object(service, "should_alert", return_value=False),
            patch(
                "backend.agents.adapters.alert_service.logger.warning"
            ) as mock_warning,
        ):
