...
class TestAgentFactory:
    ...
    async def test_create_agent_with_custom_modules(
        ...
        with patch.object(
            ...
            # Return None to simulate custom agent not found, should fallback
    ...
    async def test_create_agent_with_initialization_error(
        ...
        with patch.object(
            ...
            # Return None to use fallback from AGENT_REGISTRY
            mock_get_class.return_value = None
            agent = agent_factory_with_services.create_agent(
                agent_type,
                vector_store=mock_vector_store,
            ...
            # Sprawdzamy, że agent jest instancją EnhancedOCRAgent z AGENT_REGISTRY
            from backend.agents.anti_hallucination.enhanced_ocr_agent import (
                EnhancedOCRAgent,
            )
