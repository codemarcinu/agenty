...
async def test_orchestrator_health_check(
    ...
    # Assert
    assert result.success is True
    assert result.text == "Orchestrator is healthy"
...
async def test_orchestrator_process_file(
    ...
    with patch.object(
        orchestrator.intent_detector, "detect_intent", return_value=Mock()
    ):
        # Mock dla agent_router.route_to_agent
        with patch.object(
            orchestrator.agent_router, "route_to_agent", return_value=Mock()
        ) as mock_route:
            # Mock dla profile_manager i memory_manager
            with (
                patch.object(orchestrator.profile_manager, "get_or_create_profile"),
                patch.object(orchestrator.profile_manager, "log_activity"),
                patch.object(
                    orchestrator.memory_manager, "get_context", return_value=Mock()
                ),
...
async def test_orchestrator_process_file_unsupported_type(
    ...
    # Assert
    assert result.success is False
    assert result.error is not None and "Unsupported content type" in result.error
    mock_profile_manager.log_activity.assert_called_once()
